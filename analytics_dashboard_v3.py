...
def chart_scale(max_value):
    ...
    if max_value <= 6:
        return max_value or 1, list(range(0, (max_value or 1) + 1))

    # Aim for about six intervals, then use a conventional 1/2/5 x 10^n step.
    raw_step = max_value / 6
    magnitude = 10 ** math.floor(math.log10(raw_step))
    normalized = raw_step / magnitude
    if normalized <= 1: nice_step = 1
    elif normalized <= 2: nice_step = 2
    elif normalized <= 5: nice_step = 5
    else: nice_step = 10
    step = max(1, int(nice_step * magnitude))
    chart_max = max(step, math.ceil(max_value / step) * step)
    return chart_max, list(range(0, chart_max + step, step))
...
def report(period, content_page=1, source_page=1):
    ...
    try:
        ...
        unique = int(rowval(conn.execute(
        ...
        rows = conn.execute(f"""
            ...
            FROM page_views pv
            LEFT JOIN published_posts pp ON pp.id=pv.content_id AND (
                pv.page_type IN ('post','member_post') OR pv.path LIKE '/blog/post/%' OR pv.path LIKE '/kwsnyderwriting/post/%')
            LEFT JOIN manuscript_books mb ON mb.id=pv.content_id AND (
                pv.page_type='novel' OR pv.path LIKE '/kwsnyderwriting/novel/%') AND pv.path NOT LIKE '%/chapter/%'
            LEFT JOIN manuscript_chapters mc ON mc.id=pv.content_id AND (
                pv.page_type='chapter' OR pv.path LIKE '/kwsnyderwriting/novel/%/chapter/%')
            {where}
        ...
        content = []
        for row in rows:
            path = clean_path(rowval(row, 0, "path")); typ = rowval(row, 1, "page_type")
            cat = rowval(row, 3, "category"); title = rowval(row, 4, "title"); effective = typ
            ...
            content.append({
                ...
                "category": CATEGORIES.get(cat, "K. W. Snyder Writing" if str(cat).lower() == "journal" else (cat or LABELS.get(path, "Home"))),
        ...
        source_rows = conn.execute(f"""
            ...
            FROM page_views pv {where}
            GROUP BY traffic_source,referrer ORDER BY views DESC,referrer
        """, params).fetchall()
        details = []; source_totals = {}
        for row in source_rows:
            name = source_label(rowval(row, 0, "source"), rowval(row, 1, "referrer"))
            ref = str(rowval(row, 1, "referrer") or "").strip() or (
                "No referrer recorded (direct/bookmark or privacy-hidden referrer)" if name == "Direct" else "Referrer not recorded")
            views = int(rowval(row, 2, "views") or 0); uv = int(rowval(row, 3, "unique_visitors") or 0)
            details.append({"source": name, "referrer": ref, "views": views, "unique_visitors": uv})
        ...
        counts = {str(rowval(r, 0, "subscription_status") or "inactive"): int(rowval(r, 1, "count") or 0)
                  for r in conn.execute("SELECT subscription_status,COUNT(*) AS count FROM members GROUP BY subscription_status").fetchall()}
        new_last_30 = int(rowval(conn.execute(
            "SELECT COUNT(*) AS count FROM members WHERE date_created IS NOT NULL AND date_created >= ?",
            [(datetime.now(EASTERN) - timedelta(days=30)).isoformat()]).fetchone(), 0, "count") or 0)

        sub_params = []; sub_where = ""
        if start:
            sub_where = " WHERE date_started IS NOT NULL AND date_started >= ?"; sub_params = [start.isoformat()]
        ...
        if start:
            cancel_where += " AND date_ends >= ?"; cancel_params = [start.isoformat()]
        ...
        return {
            ...
            "members": {
                "total": sum(counts.values()), "active": counts.get("active", 0), "past_due": counts.get("past_due", 0),
                "paused": counts.get("paused", 0), "cancelled": counts.get("cancelled", 0),
                "expired": counts.get("expired", 0), "inactive": counts.get("inactive", 0), "new_last_30_days": new_last_30,
            },
            "subscription_activity": {"new": new_subs, "cancelled_or_expired": cancelled},
        }
    finally:
        conn.close()
...
def register(app):
    ...
    def analytics_ui_v3(response):
        ...
        text = text.replace("Journal", "K. W. Snyder Writing").replace("Homepage", "Home")
