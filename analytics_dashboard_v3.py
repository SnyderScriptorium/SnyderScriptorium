...
LABELS = {
    ...
    '/blog/bookcurations': 'Book Curations', '/blog/bookreviews': 'Book Reviews',
    '/blog/curiosity_cabinet': 'Curiosity Cabinet', '/kwsnyderwriting': 'K. W. Snyder Writing',
    '/kwsnyderwriting/membership': 'K. W. Snyder Writing Membership',
}
CATEGORIES = {
    'curations': 'Book Curations', 'reviews': 'Book Reviews',
    'curiosity': 'Curiosity Cabinet', 'kwsnyderwriting': 'K. W. Snyder Writing',
...
def report(period):
    ...
    if start:
        where=' WHERE pv.viewed_at >= ?'; params=[start.isoformat()]
    try:
        ...
        daily=[]; buckets={}
        rows=conn.execute(f'SELECT pv.viewed_at AS viewed_at FROM page_views pv{where}', params).fetchall()
        for row in rows:
        ...
        daily=[{'day':k,'views':buckets[k],'visitors':0} for k in sorted(buckets)]
        ...
        content=[]
        for row in rows:
            path=rowval(row,0,'path'); typ=rowval(row,1,'page_type'); raw_category=rowval(row,3,'category'); title=rowval(row,4,'title')
            content.append({'title':label(path,typ,raw_category,title),'category':CATEGORIES.get(raw_category,'K. W. Snyder Writing' if str(raw_category).lower()=='journal' else (raw_category or 'Home')),'content_type':typ,'views':int(rowval(row,5,'views') or 0),'unique_visitors':int(rowval(row,6,'unique_visitors') or 0)})
        ...
        return {'period':period,'total_views':total,'total_views_today':total if period=='day' else None,'unique_visitors':unique,'all_time_views':all_time,'daily_views':daily,'content_views':content,'members':{'total':sum(counts.values()),'active':counts.get('active',0),'past_due':counts.get('past_due',0),'paused':counts.get('paused',0),'cancelled':counts.get('cancelled',0),'expired':counts.get('expired',0),'inactive':counts.get('inactive',0)}}
    finally:
        conn.close()
