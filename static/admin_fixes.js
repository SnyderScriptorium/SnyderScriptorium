(function () {
  ...
  function focusEditorPreservingSelection(editor) {
    if (!editor) return false;
    const restored = restoreSelection(editor);
    editor.focus({ preventScroll: true });
    if (restored) restoreSelection(editor);
  ...
  document.addEventListener('mousedown', function (event) {
    const control = event.target.closest && event.target.closest('.toolbar button, .toolbar select, .toolbar input[type="color"]');
    if (!control) return;
  ...
  function blockForCaret(editor) {
    const sel = window.getSelection();
    if (!sel || !sel.rangeCount || !selectionInside(editor)) return null;
    let node = sel.anchorNode;
    if (node && node.nodeType === Node.TEXT_NODE) node = node.parentElement;
    if (!node || !node.closest) return null;
    const block = node.closest('p,h1,h2,h3,h4,h5,h6,blockquote,pre,div,li');
  ...
  function selectedBlocks(editor) {
    const sel = window.getSelection();
    if (!sel || !sel.rangeCount || !selectionInside(editor)) return [];
    const blocks = Array.from(editor.querySelectorAll('p,h1,h2,h3,h4,h5,h6,blockquote,pre,div,li'));
    const range = sel.getRangeAt(0);
    const matches = blocks.filter(block => {
      try { return range.intersectsNode(block); } catch (_) { return false; }
    });
    if (matches.length) return matches;
    const single = blockForCaret(editor);
    return single ? [single] : [];
  }

  function indentBlocks(editor, outdent) {
    if (!editor) return;
    ...
    blocks.forEach(block => {
      if (block.tagName === 'LI') {
        const current = parseFloat(getComputedStyle(block).marginLeft) || 0;
        const next = Math.max(0, current + (outdent ? -amount : amount));
        block.style.marginLeft = `${next}em`;
      } else {
  ...
  function preserveScroll(editor, callback) {
    ...
    const editorLeft = editor.scrollLeft;
    callback();
    window.scrollTo(pageX, pageY);
    editor.scrollTop = editorTop;
    editor.scrollLeft = editorLeft;
  ...
  document.addEventListener('keydown', function (event) {
    ...
    if (event.key === 'Tab') {
      ...
      preserveScroll(editor, function () {
        indentBlocks(editor, event.shiftKey);
      });
      rememberSelection(editor);
      return;
    }
  ...
  function sanitizePastedHtml(html) {
    ...
    const allowed = new Set([
      'A','B','STRONG','I','EM','U','S','SPAN','P','DIV','BR',
      'H1','H2','H3','H4','H5','H6','BLOCKQUOTE','PRE','OL','UL','LI','FONT','SUB','SUP'
    ]);
    const styleProps = [
      'font-family','font-size','color','background-color','font-weight',
      'font-style','text-decoration','text-align','vertical-align',
      'margin-left','margin-right','text-indent','line-height'
    ];
    ...
    function clean(node) {
      ...
      Array.from(node.attributes).forEach(attr => {
        ...
        if (name !== 'style' && !(node.tagName === 'A' && name === 'href')) node.removeAttribute(attr.name);
      });
  ...
  function installToolbar(toolbar) {
    ...
    const editor = toolbar.nextElementSibling && toolbar.nextElementSibling.matches(EDITOR_SELECTOR)
      ? toolbar.nextElementSibling : null;
    if (!editor) return;
    ...
    if (sizeSelect) {
      ...
      [8,9,10,11,12,14,16,18,20,22,24,26,28,32,36,40,44,48,54,60,64].forEach(pt => {
        addOption(sizeSelect, String(pt), `${pt}pt`);
      });
      sizeSelect.value = '';
    ...
    markerSize.addEventListener('change', function () {
      setMarkerSize(editor, this.value);
      this.value = 'match';
    });
    ...
    markerColor.value = '#24333B';
    markerColor.title = 'Number color for the selected numbered list';
    markerColor.setAttribute('aria-label', 'Number color');
  ...
  function organizePublishedPosts() {
    ...
    const labels = [
      'Book Curations','Book Reviews','Curiosity Cabinet',
      'K. W. Snyder Writing — Essays','K. W. Snyder Writing — Short Stories',
      'K. W. Snyder Writing — Poems','K. W. Snyder Writing — Vignettes'
    ];
    cards.forEach(card => {
    ...
    const show = category => {
      cards.forEach(card => {
        card.style.display = category === 'All Posts' || card.dataset.category === category ? '' : 'none';
      });
    ...
    categories.forEach(category => {
      ...
      badge.textContent = ` (${category === 'All Posts' ? cards.length : (counts[category] || 0)})`;
