(function () {
  ...
  const DEFAULT_COLORS = [
    '#24333B','#5C4033','#75695D','#8A6A4A','#A67B5B',
    '#6F6257','#7A2929','#405A32','#2F4F4F','#4B3A5A',
    '#7B4B3A','#8B6F47','#000000','#444444','#666666','#FFFFFF'
  ];
  ...
  function getRecentColors() {
    try {
      ...
      if (Array.isArray(stored)) return stored.filter(c => /^#[0-9A-F]{6}$/i.test(c)).slice(0, 12);
    } catch (_) {}
    return [];
  }
  ...
  function saveRecentColor(color) {
    ...
    let recent = getRecentColors().filter(c => c !== normalized);
    recent.unshift(normalized);
    recent = recent.slice(0, 12);
    try { localStorage.setItem(COLOR_STORAGE_KEY, JSON.stringify(recent)); } catch (_) {}
    return recent;
  }
  ...
  function swatch(color, title, onClick) {
    ...
    button.type = 'button';
    button.className = 'snyder-color-swatch';
    button.title = title + ' ' + color;
    button.setAttribute('aria-label', title + ' ' + color);
    button.style.cssText = 'width:24px!important;height:24px!important;min-width:24px;padding:0!important;border:1px solid #8f816d;border-radius:4px;background:' + color + ';cursor:pointer;box-shadow:inset 0 0 0 1px rgba(255,255,255,.35);';
    button.addEventListener('mousedown', e => e.preventDefault());
    button.addEventListener('click', onClick);
    return button;
  }

  function renderRecent(container, colors, apply) {
    container.innerHTML = '';
    if (!colors.length) {
      const empty = document.createElement('span');
      empty.textContent = 'None yet';
      empty.style.cssText = 'font-size:.8rem;color:var(--muted);';
      container.appendChild(empty);
      return;
    }
    colors.forEach(color => container.appendChild(swatch(color, 'Recent color', () => apply(color))));
  }
  ...
  function addColor(toolbar, editor, sizeSelect) {
    ...
    wrap.style.cssText = 'position:relative;display:inline-flex;align-items:center;gap:4px;margin:0 2px;';

    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'snyder-color-button';
    button.setAttribute('aria-label','Font color');
    button.title = 'Font color — choose from the palette or pick a custom color';
    button.style.cssText = 'width:auto!important;min-width:58px;height:30px;padding:4px 8px!important;display:inline-flex;align-items:center;gap:5px;';

    const label = document.createElement('span');
    label.textContent = 'Color';
    const indicator = document.createElement('span');
    indicator.style.cssText = 'width:18px;height:18px;border:1px solid #8f816d;border-radius:3px;background:#24333B;display:inline-block;';
    button.append(label, indicator);

    const menu = document.createElement('div');
    menu.className = 'snyder-color-menu';
    menu.style.cssText = 'display:none;position:absolute;z-index:10000;top:36px;left:0;width:270px;padding:12px;background:#FFFDF8;border:1px solid var(--line);border-radius:7px;box-shadow:0 6px 18px rgba(40,30,20,.18);';

    const title = document.createElement('div');
    title.textContent = 'Font Color';
    title.style.cssText = 'font-weight:600;color:var(--brown);margin-bottom:8px;';
    menu.appendChild(title);

    const paletteTitle = document.createElement('div');
    paletteTitle.textContent = 'Color Palette';
    paletteTitle.style.cssText = 'font-size:.8rem;color:var(--muted);margin-bottom:5px;';
    menu.appendChild(paletteTitle);

    const palette = document.createElement('div');
    palette.style.cssText = 'display:flex;flex-wrap:wrap;gap:5px;margin-bottom:10px;';
    menu.appendChild(palette);

    const recentTitle = document.createElement('div');
    recentTitle.textContent = 'Recent Colors';
    recentTitle.style.cssText = 'font-size:.8rem;color:var(--muted);margin-bottom:5px;';
    menu.appendChild(recentTitle);

    const recent = document.createElement('div');
    recent.style.cssText = 'display:flex;flex-wrap:wrap;gap:5px;margin-bottom:10px;';
    menu.appendChild(recent);

    const customRow = document.createElement('div');
    customRow.style.cssText = 'display:flex;align-items:center;gap:8px;border-top:1px solid var(--line);padding-top:9px;';
    const customLabel = document.createElement('span');
    customLabel.textContent = 'Custom';
    customLabel.style.cssText = 'font-size:.8rem;color:var(--muted);';
    const input = document.createElement('input');
    input.type = 'color'; input.value = '#24333B';
    input.setAttribute('aria-label','Choose custom font color');
    input.style.cssText = 'width:40px!important;height:30px!important;padding:2px!important;cursor:pointer;';
    customRow.append(customLabel, input);
    menu.appendChild(customRow);

    function applyColor(color) {
      editor.focus(); restore(editor);
      command(editor, 'foreColor', color);
      indicator.style.background = color;
      const colors = saveRecentColor(color);
      renderRecent(recent, colors, applyColor);
      menu.style.display = 'none';
    }

    DEFAULT_COLORS.forEach(color => palette.appendChild(swatch(color, 'Palette color', () => applyColor(color))));
    renderRecent(recent, getRecentColors(), applyColor);

    button.addEventListener('mousedown', () => remember(editor));
    ...
    if (sizeSelect) sizeSelect.insertAdjacentElement('afterend', wrap);
