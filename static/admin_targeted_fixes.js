(function () {
  ...
  function setPostEditorCategoryMode(){
    const category=document.getElementById('postCategory'),wrapper=document.getElementById('postKWSubcategoryWrap'),sub=document.getElementById('postKWSubcategory');
    if(!category||!wrapper||!sub)return;
    const oldValue=category.value;
    const oldKW=KW_SUBCATEGORIES.includes(oldValue)?oldValue:(oldValue==='journal'?'kwsnyderwriting':null);
    category.innerHTML='';
    ...
    function sync(){
      ...
      if(isKW){
        if(!KW_SUBCATEGORIES.includes(sub.value))sub.value=rememberedKWSubcategory||'kwsnyderwriting';
        rememberedKWSubcategory=sub.value;
      }
      const access=document.getElementById('postAccess');
      if(access){access.value=isKW?'members':'public';access.disabled=isKW;}
    }
    category.addEventListener('change',sync);
    sub.addEventListener('change',()=>{rememberedKWSubcategory=sub.value;sync();});
    sync();
  }
  ...
  function preserveKWSubcategoryAfterDraftLoad(){
    ...
    const apply=()=>{
      ...
      if(isKW){
        ...
        sub.value=rememberedKWSubcategory;wrapper.classList.remove('hidden');
    ...
    apply();
    const observer=new MutationObserver(apply);observer.observe(category,{attributes:true,childList:true,subtree:true});observer.observe(sub,{attributes:true,childList:true,subtree:true});
  }
  ...
  function categoryForCard(card){
    ...
    if(text.includes('k. w. snyder writing')||text.includes('k.w. snyder writing')||text.includes('short stories')||text.includes('poems')||text.includes('vignettes'))return'kwsnyderwriting';
    return'other';
  }
  ...
  function enforcePublishedDisplay(){
    ...
    Array.from(list.children).forEach(card=>{
      ...
      if(categoryForCard(card)==='kwsnyderwriting'){
        const small=card.querySelector('small');
        if(small){const bits=small.textContent.split(' · ');if(bits.length)bits[bits.length-1]='Members Only';small.textContent=bits.join(' · ');}
      }
    });
  }

  function addPublishedFilters(){
    const section=document.getElementById('published'),list=document.getElementById('publishedList');
    if(!section||!list||document.getElementById('publishedFilters'))return;
    const wrap=document.createElement('div');wrap.id='publishedFilters';wrap.className='actions';wrap.style.marginBottom='8px';
    const title=document.createElement('strong');title.textContent='Show:';title.style.color='var(--brown)';wrap.appendChild(title);
    ...
    function apply(){
      ...
      cards.forEach(card=>{const c=categoryForCard(card);if(counts[c]!==undefined)counts[c]++;});
      cards.forEach(card=>card.classList.toggle('hidden',active!=='all'&&categoryForCard(card)!==active));
      wrap.querySelectorAll('button[data-filter]').forEach(button=>{const value=button.dataset.filter;button.className=value===active?'':'light';button.textContent=`${button.dataset.label} (${counts[value]||0})`;});
    }
    ...
    const note=section.querySelector('.note');if(note)note.insertAdjacentElement('afterend',wrap);else section.insertBefore(wrap,list);
    const observer=new MutationObserver(apply);observer.observe(list,{childList:true});apply();
  }

  function addSubscriberDashboardLink(){
    const tabs=document.querySelector('.tabs');if(!tabs||document.getElementById('subscriberDashboardLink'))return;
    const button=document.createElement('button');button.id='subscriberDashboardLink';button.type='button';button.className='light';button.textContent='Subscriber Dashboard';button.addEventListener('click',()=>{location.href='/admin/subscribers';});
    const analyticsTab=document.getElementById('tab-stats');if(analyticsTab)analyticsTab.insertAdjacentElement('afterend',button);else tabs.appendChild(button);
  }

  function addTodayAnalyticsButton(){
    const stats=document.getElementById('stats');if(!stats||document.getElementById('analyticsTodayButton'))return;
    const actions=stats.querySelector('.actions');if(!actions)return;
    const b=document.createElement('button');b.id='analyticsTodayButton';b.type='button';b.className='light';b.textContent='1 Day — Today';b.onclick=()=>window.loadStats&&window.loadStats('day');actions.insertBefore(b,actions.firstChild);
    const three=stats.querySelector('.three');
    if(three&&!document.getElementById('statUniqueVisitors')){const card=document.createElement('div');card.className='card';card.innerHTML='<div><h3>Unique Visitors</h3><small>Selected period</small></div><strong id="statUniqueVisitors">0</strong>';three.appendChild(card);}
  }

  function normalizeAnalyticsLabel(category,path){
    const c=String(category||'').toLowerCase();
    if(path==='/')return'Home';if(c==='site')return'Home';if(c==='journal')return'K. W. Snyder Writing';
  ...
  function installAnalyticsOverride(){
    window.loadStats=async function(period='30'){
      ...
      try{
        ...
        const heading=document.querySelector('#stats .preview h3');if(heading)heading.textContent=period==='day'?'Views by Hour — Today':'Views Over Time';
        const totalCard=statViews&&statViews.closest('.card');if(totalCard){const h3=totalCard.querySelector('h3'),small=totalCard.querySelector('small');if(h3)h3.textContent=period==='day'?'Total Views Today':'Total Views';if(small)small.textContent=period==='day'?'Today':'Selected period';}
  ...
  function installKWPublishGuard(){
    const canonical=function(){
      const c=document.getElementById('postCategory'),sub=document.getElementById('postKWSubcategory');let value=(c&&c.value)||'';
  ...
  async function repairLegacyJournalContent(){
    try{
      ...
      for(const p of posts){if(p.category!=='journal')continue;await fetch(`/api/published/${p.id}`,{method:'PUT',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({title:p.title,content:p.content,category:'kwsnyderwriting',accessLevel:'members'})});}
      const drafts=await fetch('/api/drafts',{credentials:'same-origin'}).then(r=>r.ok?r.json():[]);
      for(const d of drafts){if(d.category!=='journal')continue;await fetch(`/api/drafts/${d.id}`,{method:'PUT',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:JSON.stringify({title:d.title,content:d.content,category:'kwsnyderwriting',date:d.date})});}
    }catch(_){}
  }

  function start(){
    setPostEditorCategoryMode();
    preserveKWSubcategoryAfterDraftLoad();
    removeDuplicateSizeSelectors();
    addPublishedFilters();
    addSubscriberDashboardLink();
    addTodayAnalyticsButton();
    installKWPublishGuard();
    repairLegacyJournalContent();
