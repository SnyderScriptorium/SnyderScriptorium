...
def ensure_analytics_table():
    ...
    try:
        if using_postgres():
            ...
            conn.execute("CREATE TABLE IF NOT EXISTS page_views (id INTEGER PRIMARY KEY AUTOINCREMENT, path TEXT NOT NULL, page_type TEXT NOT NULL DEFAULT 'page', content_id INTEGER, category TEXT, viewed_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP, visitor_key TEXT)")
...
def _visitor_key():
    ...
    if key and len(key) <= 100:
        return key
    key = uuid.uuid4().hex
    g.analytics_new_visitor_key = key
    return key


def _enhanced_record_page_view(path, page_type='page', content_id=None, category=None):
    if path.startswith('/static/') or path.startswith('/api/') or path.startswith('/admin') or _is_bot():
        return
    key = _visitor_key()
    g.analytics_visitor_key = key
    conn = None
    try:
        conn = get_db()
        conn.execute("INSERT INTO page_views(path, page_type, content_id, category, visitor_key) VALUES (?, ?, ?, ?, ?)", (path, page_type, content_id, category, key))
        conn.commit()
    except Exception:
        if conn:
            try:
...
def _analytics_report(period='month'):
    ...
    try:
        ...
        unique_visitors = conn.execute(f"SELECT COUNT(DISTINCT visitor_key) FROM page_views{where} AND visitor_key IS NOT NULL" if where else "SELECT COUNT(DISTINCT visitor_key) FROM page_views WHERE visitor_key IS NOT NULL", params).fetchone()[0]
        ...
        content_rows = conn.execute(f"""
            SELECT pv.content_type, pv.content_id,
                   COALESCE(pp.title, mb.title, mc.title, pv.path) AS title,
                   COALESCE(pv.category, 'site') AS category,
                   pv.path, COUNT(*) AS views,
                   COUNT(DISTINCT pv.visitor_key) AS unique_visitors
            FROM page_views pv
            LEFT JOIN published_posts pp ON pp.id = pv.content_id AND pv.content_type IN ('post','member_post')
            LEFT JOIN manuscript_books mb ON mb.id = pv.content_id AND pv.content_type = 'novel'
            LEFT JOIN manuscript_chapters mc ON mc.id = pv.content_id AND pv.content_type = 'chapter'
            {where.replace('viewed_at', 'pv.viewed_at')}
            GROUP BY pv.content_type, pv.content_id, COALESCE(pp.title, mb.title, mc.title, pv.path), COALESCE(pv.category, 'site'), pv.path
        ...
        sub_params = []
        if start:
            sub_where = ' WHERE date_started IS NOT NULL AND date_started >= ?'
            sub_params.append(start.isoformat())
        ...
        cancel_where = " WHERE status IN ('cancelled','expired') AND date_ends IS NOT NULL"
        cancel_params = []
        if start:
            cancel_where += ' AND date_ends >= ?'
            cancel_params.append(start.isoformat())
        ...
        return {
            ...
            'daily': [{'day': row['day'], 'views': int(row['views']), 'visitors': int(row['visitors'])} for row in daily_rows],
            'categories': [{'category': _label_for(row['category']), 'raw_category': row['category'], 'views': int(row['views']), 'unique_visitors': int(row['unique_visitors'])} for row in category_rows],
            'posts': [{'content_type': row['content_type'], 'content_id': row['content_id'], 'title': row['title'], 'category': _label_for(row['category']), 'path': row['path'], 'views': int(row['views']), 'unique_visitors': int(row['unique_visitors'])} for row in content_rows],
            'members': {'total': total_members, 'active': member_counts.get('active', 0), 'past_due': member_counts.get('past_due', 0), 'paused': member_counts.get('paused', 0), 'cancelled': member_counts.get('cancelled', 0), 'expired': member_counts.get('expired', 0), 'inactive': member_counts.get('inactive', 0)},
            'subscriber_daily': [{'day': row['day'], 'new': int(row['count']), 'cancelled': 0} for row in new_rows] + [{'day': row['day'], 'new': 0, 'cancelled': int(row['count'])} for row in cancel_rows],
        }
    finally:
        conn.close()
...
def _public_category(category, template, title, filter_name):
    conn = get_db()
    labels = conn.execute("SELECT DISTINCT category_name FROM published_posts WHERE category = ? AND access_level = 'public' AND category_name IS NOT NULL AND TRIM(category_name) <> '' ORDER BY LOWER(category_name)", (category,)).fetchall()
    ...
    params = [category]
    if selected:
        sql += " AND category_name = ?"; params.append(selected)
    if search:
        sql += " AND (LOWER(title) LIKE LOWER(?) OR LOWER(content) LIKE LOWER(?))"; like = f"%{search}%"; params.extend([like, like])
...
def register_site_enhancements(app):
    ...
    # The original app-level tracker calls this module-level function by name.
    # Replacing that function gives the existing tracker anonymous visitor IDs
    # and bot filtering without registering a second competing tracker.
    try:
        app_module = sys.modules.get('app')
        if app_module is not None:
            app_module.record_page_view = _enhanced_record_page_view
    except Exception:
        pass
    ...
    def set_analytics_visitor_cookie(response):
        ...
        if key and not request.cookies.get('ss_visitor'):
            response.set_cookie('ss_visitor', key, max_age=60 * 60 * 24 * 365 * 2, httponly=True, samesite='Lax', secure=True)
        return response
    ...
    # Replace the optional analytics.py endpoint if it is already registered;
    # otherwise register the endpoint here for the production worker.
    app.view_functions['analytics_api'] = analytics_api
    if 'analytics_api' not in app.url_map._rules_by_endpoint:
        app.add_url_rule('/api/analytics', endpoint='analytics_api', view_func=analytics_api, methods=['GET'])
    ...
    def inject_admin_label_tools(response):
        if request.path.startswith('/admin') and response.content_type.startswith('text/html') and response.status_code == 200:
            body=response.get_data(as_text=True); marker='</body>'; script='<script src="/static/category_labels.js"></script>'
            if marker in body and 'category_labels.js' not in body: response.set_data(body.replace(marker,script+marker))
        return response
