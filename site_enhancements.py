...
def ensure_analytics_table():
    ...
    try:
        if using_postgres():
            conn.execute("CREATE TABLE IF NOT EXISTS page_views (id BIGSERIAL PRIMARY KEY, path TEXT NOT NULL, page_type TEXT NOT NULL DEFAULT 'page', content_id BIGINT, category TEXT, viewed_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP)")
        else:
            conn.execute("CREATE TABLE IF NOT EXISTS page_views (id INTEGER PRIMARY KEY AUTOINCREMENT, path TEXT NOT NULL, page_type TEXT NOT NULL DEFAULT 'page', content_id INTEGER, category TEXT, viewed_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)")
        conn.commit()
    finally:
        conn.close()
...
def _label_for(category, supplied=None):
    ...
    if category in defaults: return supplied or defaults[category]
    return supplied or {'kwsnyderwriting':'K. W. Snyder Writing','kw_short_stories':'Short Stories','kw_poems':'Poems','kw_vignettes':'Vignettes'}.get(category, 'Journal')
