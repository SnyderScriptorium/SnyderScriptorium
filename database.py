...
class PostgresConnection:
    ...
    def _translate(self, sql): return sql.replace("?", "%s")
    def execute(self, sql, params=()):
        ...
        if normalized.upper().startswith("PRAGMA TABLE_INFO("):
            ...
            cursor.execute("SELECT column_name AS name FROM information_schema.columns WHERE table_schema = 'public' AND table_name = %s ORDER BY ordinal_position", (table,))
            return PostgresCursor(cursor)
        sql = self._translate(sql); upper = normalized.upper()
        wants_id = upper.startswith("INSERT INTO") and any(f"INSERT INTO {table}" in upper for table in ("DRAFTS","PUBLISHED_POSTS","MANUSCRIPTS","MANUSCRIPT_BOOKS","MANUSCRIPT_CHAPTERS","MEMBERS","SUBSCRIPTIONS")) and "RETURNING" not in upper
        if wants_id: sql = sql.rstrip().rstrip(";") + " RETURNING id"
        cursor = self._connection.cursor(); cursor.execute(sql, params); result = PostgresCursor(cursor)
        if wants_id:
            row = cursor.fetchone(); result._lastrowid = row["id"] if row else None
        return result
...
def get_db():
    if not using_postgres():
        conn = sqlite3.connect(DATABASE); conn.row_factory = sqlite3.Row; conn.execute("PRAGMA foreign_keys = ON"); return conn
    import psycopg
    from psycopg.rows import dict_row
    conn = psycopg.connect(DATABASE_URL, row_factory=dict_row); conn.execute("SET TIME ZONE 'UTC'"); return PostgresConnection(conn)
...
def postgres_init_db(conn):
    statements = [
        ...
        "CREATE TABLE IF NOT EXISTS manuscripts (id BIGSERIAL PRIMARY KEY,title TEXT NOT NULL,content TEXT NOT NULL,date_created TEXT NOT NULL)",
        ...
        "CREATE TABLE IF NOT EXISTS site_content (key TEXT PRIMARY KEY,value TEXT NOT NULL DEFAULT '',updated_at TEXT NOT NULL)",
    ...
    conn.execute("""CREATE OR REPLACE FUNCTION force_kw_members_access() RETURNS trigger AS $$ BEGIN IF NEW.category IN ('kwsnyderwriting','kw_short_stories','kw_poems','kw_vignettes') THEN NEW.access_level := 'members'; END IF; RETURN NEW; END; $$ LANGUAGE plpgsql""")
    conn.execute("DROP TRIGGER IF EXISTS trg_force_kw_members_access ON published_posts")
    conn.execute("""CREATE TRIGGER trg_force_kw_members_access BEFORE INSERT OR UPDATE OF category, access_level ON published_posts FOR EACH ROW EXECUTE FUNCTION force_kw_members_access()""")
    conn.execute("INSERT INTO site_content(key,value,updated_at) VALUES (%s,%s,%s) ON CONFLICT(key) DO NOTHING",("about_content","The Snyder Scriptorium is a growing home for books, writing, curiosity, and the stories that bring people together.","01/01/1970 12:00 AM"))
    conn.commit()

def init_db():
    conn = get_db()
    if using_postgres(): postgres_init_db(conn); conn.close(); return
    ...
    conn.execute("""CREATE TRIGGER IF NOT EXISTS trg_force_kw_members_access_insert AFTER INSERT ON published_posts WHEN NEW.category IN ('kwsnyderwriting','kw_short_stories','kw_poems','kw_vignettes') AND NEW.access_level != 'members' BEGIN UPDATE published_posts SET access_level='members' WHERE id=NEW.id; END""")
    conn.execute("""CREATE TRIGGER IF NOT EXISTS trg_force_kw_members_access_update AFTER UPDATE OF category, access_level ON published_posts WHEN NEW.category IN ('kwsnyderwriting','kw_short_stories','kw_poems','kw_vignettes') AND NEW.access_level != 'members' BEGIN UPDATE published_posts SET access_level='members' WHERE id=NEW.id; END""")
    ...
    for col, definition in (("visitor_key","TEXT"),("referrer","TEXT"),("traffic_source","TEXT")):
        existing={r["name"] for r in conn.execute("PRAGMA table_info(page_views)").fetchall()}
        if col not in existing: conn.execute(f"ALTER TABLE page_views ADD COLUMN {col} {definition}")
    conn.commit(); conn.close()

def migrate_sqlite_to_postgres():
    if not using_postgres() or not os.path.exists(DATABASE): return False
    sqlite_conn=sqlite3.connect(DATABASE); sqlite_conn.row_factory=sqlite3.Row; pg=get_db()
    try:
        for table in ["drafts","published_posts","manuscripts","manuscript_books","manuscript_chapters","members","subscriptions","site_content"]:
            for row in sqlite_conn.execute(f"SELECT * FROM {table}").fetchall():
                data=dict(row); columns=list(data.keys()); values=[data[c] for c in columns]; placeholders=", ".join(["%s"]*len(columns)); quoted=", ".join(columns)
                if table=="site_content": sql=f"INSERT INTO {table} ({quoted}) VALUES ({placeholders}) ON CONFLICT(key) DO UPDATE SET value=EXCLUDED.value,updated_at=EXCLUDED.updated_at"
                else: sql=f"INSERT INTO {table} ({quoted}) VALUES ({placeholders}) ON CONFLICT DO NOTHING"
                pg.execute(sql,values)
        pg.commit(); sqlite_conn.close(); pg.close(); return True
    except Exception:
        pg.rollback(); sqlite_conn.close(); pg.close(); raise
